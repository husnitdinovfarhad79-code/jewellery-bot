...
def init_db():
    ...
    cursor = conn.cursor()
    # Добавлена колонка telegram_id для разделения заказов между пользователями
    cursor.execute('''
...
async def export_to_excel(message: Message):
    ...
    conn = sqlite3.connect('jewelry_orders.db')
    # Фильтруем выгрузку по telegram_id текущего пользователя
    ...
    # Убираем системные колонки, чтобы отчет был красивым
...
async def save_order_to_db(photo_id, message: Message, state: FSMContext):
    ...
    conn = sqlite3.connect('jewelry_orders.db')
    cursor = conn.cursor()
    # Сохраняем message.from_user.id в таблицу
    cursor.execute('''
...
# --- СЦЕНАРИЙ: РЕДАКТИРОВАНИЕ ЗАКАЗА (ТОЛЬКО СВОЕГО) ---
...
async def process_edit_id(message: Message, state: FSMContext):
    try:
        ...
        conn = sqlite3.connect('jewelry_orders.db')
        cursor = conn.cursor()
        # Добавлена проверка на telegram_id, чтобы нельзя было редактировать чужие заказы
...
async def process_new_value(message: Message, state: FSMContext):
    ...
    conn = sqlite3.connect('jewelry_orders.db')
    cursor = conn.cursor()
    # Дополнительная безопасность: проверка telegram_id при записи изменений
...
# --- СЦЕНАРИЙ: ЗАВЕРШЕНИЕ ЗАКАЗА (ТОЛЬКО СВОЕГО) ---
...
async def process_close_id(message: Message, state: FSMContext):
    try:
        ...
        conn = sqlite3.connect('jewelry_orders.db')
        cursor = conn.cursor()
        # Проверяем, принадлежит ли закрываемый заказ этому telegram_id
...
async def finalize_order(end_photo_id, message: Message, state: FSMContext):
    ...
    conn = sqlite3.connect('jewelry_orders.db')
    cursor = conn.cursor()
    # Проверяем telegram_id при закрытии заказа
    cursor.execute('''
        UPDATE orders 
...
async def main():
    init_db()
    print("Бот запущен в многопользовательском режиме!")
